...
def generate_next_demand(length):
    #sample = np.random.uniform(low=0, high=5, size=(rows, columns))
    fr = 0
    to = 2.0
    distinct_value = np.arange(fr, to + 0.1, 0.2) #to + 0.1 so we can include 2 in the list
    sample = itertools.product(distinct_value, repeat=length)
    return sample
...
if __name__ == "__main__":
    ...
    list_of_tuples = [(i, j) for i in range(1, 15) for j in range(1, 15) if i != j]
    current_demands_object = generate_next_demand(182)

    f = open("OutputFileInCSV", "w")
    writer = csv.writer(f)
    tic = time.time() # measure how much time the for loop take.
    for i in range(10):
        #dd = next(current_demands_object)
        dd = random_product(np.arange(0, 0.1, 0.01), repeat=182)
        #print(dd)
        ...
        if LpStatus[prob.status] == "Infeasible":
            print(colored("Infeasible found!", 'yellow'), "@ time: ", time.time())
            continue
        ...
        for v in prob.variables():
            ...
            '''if v.varValue >= 2 or v.varValue < 0:
                #colored('hello', 'red')
        ...
        #print(templist)
        #print(dd)
        one_sample = list(dd) + templist
        #print(one_sample)
        writer.writerows([one_sample])
